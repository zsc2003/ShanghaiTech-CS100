...
struct brainfuck_state *brainfuck_state_new()
{
 	...
 	state->memory_buffer = calloc(30000, sizeof(uint8_t));
...
void brainfuck_execute_left(struct brainfuck_state *state, char **src)
{
	if (state->memory_buffer[state->memory_pointer_offset] == 0)
	{
		int unmatched_depth = 0;
		while (1)
	    {
			(*src)++;
			if ((**src) == '\0' || (**src) == '\n')
...
void brainfuck_execute_right(struct brainfuck_state *state, char **src)
{
	if (state->memory_buffer[state->memory_pointer_offset] != 0)
	{
		int unmatched_depth = 0;
 ...
 */
bool brainfuck_main(struct brainfuck_state *state, char *src, brainfuck_input_handler input_handler, brainfuck_output_handler output_handler)
{
	...
	while ((*s) != '\0' && (*s) != '\n')
	{
		...
		{
			brainfuck_execute_left(state, &s);
		}
		else if ((*s) == ']')
		{
			brainfuck_execute_right(state, &s);
		}
		s++;
	}
	if(*s == '\0' || *s == '\n')

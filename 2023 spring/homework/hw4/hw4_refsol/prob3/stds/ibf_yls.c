...
struct brainfuck_state {
  ...
  size_t loop_buffer_size;       /* The loop buffer's size. */
...
struct brainfuck_state* brainfuck_state_new() {
  ...
  new_state->loop_buffer = (char*)calloc(BRAINFUCK_LOOP_BUFFER_SIZE, sizeof(char));
  new_state->loop_buffer_size = 0;
 ...
 */
void brainfuck_execute(struct brainfuck_state* state, char* src,
  ...
  while (curr < len) {
    ...
    case BRAINFUCK_TOKEN_LOOP_START:
      if (state->memory_buffer[state->memory_pointer_offset] == 0) {
        ...
        while (unmatched_depth != 0) {
          curr++;
          if (src[curr] == BRAINFUCK_TOKEN_LOOP_START) {
            unmatched_depth++;
          }
          else if (src[curr] == BRAINFUCK_TOKEN_LOOP_END) {
            unmatched_depth--;
          }
        }
      }
    ...
    case BRAINFUCK_TOKEN_LOOP_END:
      if (state->memory_buffer[state->memory_pointer_offset] != 0) {
        ...
        while (unmatched_depth != 0) {
          curr--;
          if (src[curr] == BRAINFUCK_TOKEN_LOOP_END) {
            unmatched_depth++;
          }
          else if (src[curr] == BRAINFUCK_TOKEN_LOOP_START) {
            unmatched_depth--;
          }
        }
      }
...
bool brainfuck_main(struct brainfuck_state* state, char* src,
  brainfuck_input_handler input_handler, brainfuck_output_handler output_handler) {
  brainfuck_execute(state, src, input_handler, output_handler);
  // size_t len = strlen(src);
  // if (state->unmatched_depth > 0) {
  //   size_t curr = 0;
  //   while (curr < len) {
  //     if (src[curr] == BRAINFUCK_TOKEN_LOOP_START) {
  //       state->unmatched_depth += 1;
  //     }
  //     else if (src[curr] == BRAINFUCK_TOKEN_LOOP_END) {
  //       state->unmatched_depth -= 1;
  //     }
  //     state->loop_buffer[state->loop_buffer_size] = src[curr];
  //     state->loop_buffer_size += 1;
  //     if (state->unmatched_depth == 0) {
  //       // add a null terminator to make it a string
  //       state->loop_buffer[state->loop_buffer_size + 1] = '\0';
  //       brainfuck_execute(state, state->loop_buffer,
  //         input_handler, output_handler);
  //       state->loop_buffer_size = 0;
  //       curr += 1;
  //       if (curr < len) {
  //         brainfuck_main(state, src + curr, input_handler, output_handler);
  //       }
  //       break;
  //     }
  //     curr += 1;
  ...
  // else {
  //   size_t curr = 0;
  //   char* code_buffer = (char*)calloc(BRAINFUCK_LOOP_BUFFER_SIZE, sizeof(char));
  ...
  //     }
  //     else {
  //       code_buffer[code_buffer_size] = src[curr];
  //       code_buffer_size += 1;
  //       curr += 1;
  //     }
  //   }
  //   brainfuck_execute(state, code_buffer,
  //         input_handler, output_handler);
  //   free(code_buffer);
  // }
  return true;

...
Matrix::Matrix(Index rows, Index cols)
    : m_rows(rows), m_cols(cols), m_data(new Scalar[rows * cols]{}) {}
...
Matrix &Matrix::operator=(const Matrix &other) {
  Scalar *new_data = new Scalar[other.rows() * other.cols()];
  std::copy(other.m_data, other.m_data + other.rows() * other.cols(), new_data);
  m_rows = other.rows();
  m_cols = other.cols();
  delete[] m_data;
  m_data = new_data;
